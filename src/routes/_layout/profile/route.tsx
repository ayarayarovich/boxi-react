        ...
        <div className='relative flex h-full flex-col items-stretch pt-2 pb-5'>
            ...
            <div className={cn('mb-5 flex items-baseline px-5', '*:flex-1 *:text-center')}>
                <Link to='/profile' activeOptions={{ exact: true }}>
                    {({ isActive }) => (
                        <div>
                            <div className={cn('py-4 opacity-30 transition-all', isActive && 'font-semibold opacity-100')}>Stats</div>
                            {isActive && navigationUnderline}
                        </div>
                    )}
                </Link>
                <Link to='/profile/referrals' activeOptions={{ exact: true }}>
                    {({ isActive }) => (
                        <div>
                            <div className={cn('py-4 opacity-30 transition-all', isActive && 'font-semibold opacity-100')}>Referrals</div>
                            {isActive && navigationUnderline}
                        </div>
                    )}
                </Link>
